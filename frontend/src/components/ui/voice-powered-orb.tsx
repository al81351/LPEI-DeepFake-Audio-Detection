...
interface VoicePoweredOrbProps {
  className?: string;
  hue?: number;
  /** External audio level (0–1). When provided, skips internal mic entirely. */
  audioLevel?: number;
...
export const VoicePoweredOrb: FC<VoicePoweredOrbProps> = ({
  ...
  // Keep a ref to the latest audioLevel so the animation loop reads it without deps
  ...
  useEffect(() => {
    ...
    try {
      ...
      glContext = rendererInstance.gl as OGLRenderingContext;
      ...
      const update = (t: number) => {
        ...
        // Determine voice level: external level takes priority over internal mic
        const extLevel = audioLevelRef.current;
        let voiceLevel = 0;

        if (extLevel !== undefined) {
          voiceLevel = extLevel;
        } else if (enableVoiceControl && isMicInitialized) {
      ...
      return () => {
        ...
        if (container && glContext?.canvas && container.contains(glContext.canvas)) {
          try { container.removeChild(glContext.canvas); } catch { /* ignore */ }
        }
        stopMicrophone();
